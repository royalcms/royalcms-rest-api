...
use RC_Error;
use RC_Hook;
use Royalcms\Component\RestApi\Controllers\IndexController;
use Royalcms\Component\RestApi\EndPoints\ApiEndPoint;
...
class RESTServer
{
    ...
    {
        $this->add_endpoint('/', array(
				'callback' => array( IndexController::class, 'get_index' ),
				'methods'  => 'GET',
				'args'     => array(
					'context' => array(
						'default' => 'view',
...
//		);
	...
	public function add_endpoint($name, $data)
    {
        $point = (new ApiEndPoint())->setCallback($data['callback'])
            ->setMethods($data['methods'])
            ->setArgs($data['args']);

        $this->endpoints[$name] = $point;
    }
	 ...
	 * @since 4.4.0
	 *
	 * @return \RC_Error|null WP_Error indicates unsuccessful login, null indicates successful
	...
	public function check_authentication()
    {
		 ...
		 * @since 4.4.0
		 *
		 * @param \RC_Error|null|true $errors WP_Error if authentication error, null if authentication
		 *                                   method wasn't used, true if authentication succeeded.
		 */
		return RC_Hook::apply_filters( 'rest_authentication_errors', null );
	}
	 ...
	 * @since 4.4.0
	 *
	 * @param \RC_Error $error WP_Error instance.
	 * @return RESTResponse List of associative arrays with code and message keys.
	 */
	protected function error_to_response( $error ) {
		...
		$response = new RESTResponse( $data, $status );
	...
	public function serve_request( $path = null )
    {
		 ...
		 */
		$send_no_cache_headers = RC_Hook::apply_filters( 'rest_send_nocache_headers', is_user_logged_in() );
		 ...
		 */
		$jsonp_enabled = RC_Hook::apply_filters( 'rest_jsonp_enabled', true );
		...
		$request = new RESTRequest( $_SERVER['REQUEST_METHOD'], $path );
		 ...
		 *
		 * @param HTTPResponse $result  Result to send to the client. Usually a WP_REST_Response.
		 * @param RESTServer   $this    Server instance.
		 * @param RESTRequest  $request Request used to generate the response.
		 */
		$result = RC_Hook::apply_filters( 'rest_post_dispatch', rest_ensure_response( $result ), $this, $request );
		 ...
		 * @param RESTServer   $this    Server instance.
		 */
		$served = RC_Hook::apply_filters( 'rest_pre_serve_request', false, $result, $request, $this );
		...
		if ( ! $served ) {
			 ...
			 * @param RESTRequest  $request Request used to generate the response.
			 */
			$result = RC_Hook::apply_filters( 'rest_pre_echo_response', $result, $this, $request );
	 ...
	 *
	 * @param RESTResponse $response Response object.
	 ...
	 * @since 4.4.0
	 *
	 * @param RESTResponse $response Response to extract links from.
		...
		foreach ( $data['_links'] as $rel => $links ) {
			...
			foreach ( $links as $item ) {
				...
				if ( ! array_key_exists( $item['href'], $this->embed_cache ) ) {
					...
					$request = RESTRequest::from_url( $item['href'] );
					if ( ! $request ) {
						$embeds[] = array();
						continue;
					}
					...
					$response = RC_Hook::apply_filters( 'rest_post_dispatch', rest_ensure_response( $response ), $this, $request );
	 ...
	 * @return RESTResponse New response with wrapped data
	 */
	public function envelope_response( $response, $embed ) {
		 ...
		 * @param RESTResponse $response Original response data.
		 */
		$envelope = RC_Hook::apply_filters( 'rest_envelope_response', $envelope, $response );
	...
	public function get_routes( $namespace = '' ) {
		 ...
		 *                         `'/path/regex' => array( array( $callback, $bitmask ).
		 */
		$endpoints = RC_Hook::apply_filters( 'rest_endpoints', $endpoints );
	 ...
	 * @since 4.4.0
	 *
	 * @param RESTRequest $request Request to attempt dispatching.
	 * @return RESTResponse Response returned by the callback.
	 */
	public function dispatch( $request )
    {
		 ...
		 * @param RESTRequest $request Request used to generate the response.
		 */
		$result = RC_Hook::apply_filters( 'rest_pre_dispatch', null, $this, $request );
		...
		foreach ( $routes as $route => $handlers ) {
			...
			foreach ( $handlers as $handler ) {
				 ...
				 * @since 4.7.0
				 *
				 * @param HTTP_Response|WP_Error $response Result to send to the client. Usually a WP_REST_Response or WP_Error.
				 * @param array                     $handler  Route handler used for the request.
				 * @param RESTRequest           $request  Request used to generate the response.
				 */
				$response = RC_Hook::apply_filters( 'rest_request_before_callbacks', $response, $handler, $request );
				...
				if ( ! is_wp_error( $response ) ) {
					 ...
					 * @param array           $handler         Route handler used for the request.
					 */
					$dispatch_result = RC_Hook::apply_filters( 'rest_dispatch_request', null, $request, $route, $handler );
				 ...
				 * @param RESTRequest           $request  Request used to generate the response.
				 */
				$response = RC_Hook::apply_filters( 'rest_request_after_callbacks', $response, $handler, $request );
	 ...
	 * @since 4.4.0
	 *
	 * @param RESTRequest $request REST request instance.
	 * @return RESTResponse|RC_Error WP_REST_Response instance if the index was found,
	...
	public function get_namespace_index( $request )
    {
		 ...
		 * @param RESTRequest  $request  Request data. The namespace is passed as the 'namespace' parameter.
		 */
		return RC_Hook::apply_filters( 'rest_namespace_index', $response, $request );
	}
	...
	public function get_data_for_routes( $routes, $context = 'view' )
    {
		...
		foreach ( $routes as $route => $callbacks ) {
			 ...
			 * @param RESTRequest $request Request data. The namespace is passed as the 'namespace' parameter.
			 */
			$available[ $route ] = RC_Hook::apply_filters( 'rest_endpoints_description', $data );
		}
		 ...
		 */
		return RC_Hook::apply_filters( 'rest_route_data', $available, $routes );
	}
