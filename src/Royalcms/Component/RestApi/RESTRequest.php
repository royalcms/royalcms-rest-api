...
class RESTRequest implements ArrayAccess
{
	...
	protected function get_parameter_order()
    {
		 ...
		 * @param RESTRequest $this  The request object.
		 */
		return RC_Hook::apply_filters( 'rest_request_parameter_order', $order, $this );
	}
	 ...
	 * @return true|\RC_Error True if the JSON data was passed or no JSON data was provided, WP_Error if invalid JSON was passed.
	 */
	protected function parse_json_params()
    {
		...
		if ( null === $params && JSON_ERROR_NONE !== json_last_error() ) {
			...
			$error_data = array(
				'status'             => RC_Http::BAD_REQUEST,
				'json_error_code'    => json_last_error(),
				'json_error_message' => json_last_error_msg(),
			);

			return new RC_Error( 'rest_invalid_json', __( 'Invalid JSON body passed.' ), $error_data );
		}
	 ...
	 * @since 4.4.0
	 *
	 * @return true|\RC_Error True if parameters were sanitized, WP_Error if an error occurred during sanitization.
	 */
	 ...
	 * @since 4.4.0
	 *
	 * @return bool|\RC_Error True if there are no parameters to validate or if all pass validation,
	 ...
	 * @return RESTRequest|false WP_REST_Request object on success, false on failure.
	 */
	public static function from_url( $url )
    {
		...
		$request = false;
		if ( ! empty( $route ) ) {
			$request = new RESTRequest( 'GET', $route );
		 ...
		 * @since 4.5.0
		 *
		 * @param RESTRequest|false $request Generated request object, or false if URL
		 *                                       could not be parsed.
		 * @param string                $url     URL the request was generated from.
		 */
		return RC_Hook::apply_filters( 'rest_request_from_url', $request, $url );
	}
}
